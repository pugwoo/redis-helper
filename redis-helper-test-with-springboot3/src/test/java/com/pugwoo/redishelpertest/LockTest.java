...
public class LockTest {
	...
	public void testReentrant() throws Exception {
		...
		Thread thread = new Thread(new Runnable() {
			@Override
			public void run() {
				String uuid = redisHelper.requireLock("", randomUUidB, 10, true);
				...
				String uuid2 = redisHelper.requireLock("", randomUUidB, 10, true);
				...
				boolean succ = redisHelper.releaseLock("", randomUUidB, uuid, true);
				assert succ;
				...
				boolean succ2 = redisHelper.releaseLock("", randomUUidB, uuid, true);
				assert succ2;

				// 再继续解锁，就失败了
				boolean succ3 = redisHelper.releaseLock("", randomUUidB, uuid, true);
				assert !succ3;
			}
		});
		thread.start();
		...
		String uuid = redisHelper.requireLock("", randomUUidA, 10, true);
		assert StringTools.isNotBlank(uuid);
	...
	public void testNotReentrant() throws Exception {
		String randomUUidA = UUID.randomUUID().toString();
		String uuid = redisHelper.requireLock("", randomUUidA, 10, false);
		assert StringTools.isNotBlank(uuid);

		// 不可重入时，再拿一次，是拿不到的
		String uuid2 = redisHelper.requireLock("", randomUUidA, 10, false);
		assert StringTools.isBlank(uuid2);
	}
		...
		for(int i=0;i<THREAD;i++){
			Thread thread = new Thread(new Runnable() {
				...
				public void run() {
					...
					String lockUuid = redisHelper.requireLock(nameSpace, key, 10, true);
					if(lockUuid != null){
					...
					if(lockUuid == null){
						while (lockUuid == null){
							lockUuid = redisHelper.requireLock(nameSpace, key, 10, true);
						}
					...
					boolean succ = redisHelper.releaseLock(nameSpace,key,lockUuid, true);
					assert succ;
