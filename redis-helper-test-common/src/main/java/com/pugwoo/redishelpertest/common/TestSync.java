...
public abstract class TestSync {
    ...
    public abstract HeartbeatTestService getHeartbeatTestService();

    public abstract ThrowIfNotGetLockTestService getThrowIfNotGetLockTestService();
    ...
    public void testHeartbeat() throws Exception {
        ...
        for(int i = 0; i < 3; i++) {
            Thread t = new Thread(new Runnable() {
                @Override
                public void run() {
                    getHeartbeatTestService().longTask();
                }
            });
            t.start();
            thread.add(t);
    ...
    public void testHelloByDefaultNamespace() throws Exception {
        ...
        for(int i = 1; i <= 10; i++) {
            ...
            Thread t = new Thread(() -> {
                try {
                    do {
                        getHelloService().helloByDefaultNamespace("nick", a);
                        System.out.println("线程" + a +
                                "执行结果详情: 是否执行了方法:" + RedisSyncContext.getHaveRun());
    ...
    public void testNotThrowIfNotGetLock() throws InterruptedException {
        ...
        for(int i = 1; i <= 3; i++) {
            ...
            Thread t = new Thread(() -> {
                try {
                    while (true) {
                        getThrowIfNotGetLockTestService().notThrowIfNotGetLock(a, 1000);
    ...
    public void testThrowIfNotGetLock() throws InterruptedException {
        ...
        for(int i = 1; i <= 3; i++) {
            ...
            Thread t = new Thread(() -> {
                try {
                    while (true) {
                        getThrowIfNotGetLockTestService().throwIfNotGetLock(1000L);
                        System.out.println(DTF.format(LocalDateTime.now()) + " 线程" + a +
                                "执行结果详情: 是否执行了方法:" + RedisSyncContext.getHaveRun());
                        if(RedisSyncContext.getHaveRun()) {
                            haveRunCount.incrementAndGet();
